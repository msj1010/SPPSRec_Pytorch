...
class MinibatchIterator(object):
    def __init__(self,
        ...
        if self.training:
            ...
            self.item_transition = self.construct_interaction_adj()
    ...
    def get_add_info(self):
        self.item_popularity = pickle.load(open('H:/RecBole-GNN-main/my_code/datasets/bookdata/item_book_popularity.pkl', 'rb+'))
        self.user_followee = pickle.load(open('H:/RecBole-GNN-main/my_code/datasets/bookdata/user_book_follower_map.pkl', 'rb+'))
    ...
    def get_pop_seq(self,seq):
        seq = np.array(seq)
        pop_seq = np.zeros_like(seq,dtype=int)
        for i in range(seq.shape[0]):
            for j in range(seq.shape[1]):
                if seq[i,j] != 0:
                    pop_seq[i, j] = self.item_popularity.get(seq[i, j],0) * 100
        return pop_seq

    def get_user_followee(self,seq):
        seq = np.array(seq)
        u_followee = np.asarray([self.user_followee.get(x,0) for x in seq])
        u_followee = (u_followee * 100)
        return u_followee.astype(np.int32)
    ...
    def _batch_feed_dict(self, current_batch):
        ...
        input_pop = self.get_pop_seq(input_x)
        ...
        feed_dict['input_pop'] = torch.tensor(input_pop).to(self.device)
        feed_dict['item_transition'] = torch.tensor(self.item_transition[0]).to(self.device)
        ...
        #此处是用户节点，一个是目标用户u的朋友，一个是目标用户u朋友的朋友,可以尝试加入用户的关注数和被关注数
        ...
        layer1_followee = self.get_user_followee(samples[2])
        layer2_followee = self.get_user_followee(samples[1])
        feed_dict['layer1_followee'] = torch.tensor(layer1_followee).to(self.device)
        feed_dict['layer2_followee'] = torch.tensor(layer2_followee).to(self.device)
        ...
        session1_pop = self.get_pop_seq(support_layers_session[0])
        session2_pop = self.get_pop_seq(support_layers_session[1])
        feed_dict['session1_pop'] = torch.tensor(session1_pop).to(self.device)
        feed_dict['session2_pop'] = torch.tensor(session2_pop).to(self.device)

        #此处与support_sessions_layer1，和support_sessions_layer2相对应，分别是这两个会话的长度
        feed_dict.update(
    ...
    def construct_interaction_adj(self):
        '''
        Construct adj table used during training.
        '''
        adj = np.zeros((self.num_items+1,self.num_items+1), dtype=np.int32)
        user_ids = set(self.train_df['UserId'].values)
        for x in user_ids:
            user_inter_seq = self.train_df[self.train_df['UserId'] == x].values
            for i in range(len(user_inter_seq)):
                if i > 0:
                    adj[user_inter_seq[i,1],user_inter_seq[i-1,1]] += 1
                    adj[user_inter_seq[i-1,1], user_inter_seq[i,1]] += 1
                elif i+1 < len(user_inter_seq):
                    adj[user_inter_seq[i,1], user_inter_seq[i + 1,1]] += 1
                    adj[user_inter_seq[i+1,1], user_inter_seq[i,1]] += 1
        deg = np.zeros((self.num_items,))
        missed = 0
        # for x in self.train_df:
